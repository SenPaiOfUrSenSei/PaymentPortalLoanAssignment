...
export default function Statement() {
  ...
  const filteredStatements = (statements || []).filter(item => {
    ...
    const loanAcc = item.loanAccountNumber || ''
    const desc = item.description || ''
    const matchesSearch = loanAcc.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          desc.toLowerCase().includes(searchQuery.toLowerCase())
    return matchesFilter && matchesSearch
  })

  // Receipt navigation handler
  const handleDownload = (item) => {
    if (item.type === 'SETTLEMENT') {
      navigate('/settlement', { state: { viewReceiptLoan: item } })
    } else {
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                    {item.type === 'MANUAL_PAYMENT' && (
                      <>
                        <span>Ref ID: {item.details?.paymentRefId || 'N/A'}</span>
                        <span>Gateway: {item.details?.paymentGateway || 'N/A'}</span>
                      </>
                    )}
                    {item.type === 'SETTLEMENT' && (
                      <>
                        <span>Lender: {item.details?.billerName || 'N/A'}</span>
                        <span>Principal: {formatRupees(item.details?.principalOutstanding || 0)}</span>
                      </>
                    )}
                    {item.type === 'AUTOPAY_DEBIT' && (
                      <>
                        <span>UMN: {item.details?.umn || 'N/A'}</span>
                        <span>VPA: {item.details?.vpa || 'N/A'}</span>
                      </>
                    )}
                  </div>
                </div>
